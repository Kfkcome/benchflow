...
Importing ``benchflow.sdk`` triggers ``_patch_docker_dind()`` at sdk.py:135.
...
def smoke_jobs_dir(tmp_path: Path) -> Iterator[Path]:
    ...
    equivalent, so Docker's ``HOST_VERIFIER_LOGS_PATH`` bind mount silently
    maps to nothing — verifier writes to the bind, the host loses them, and
    ``reward.txt`` never appears. ``_patch_docker_dind`` only translates paths
